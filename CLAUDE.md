...
## What this is
A browser-based pixel-art mini-RPG built in Phaser 3. The PLAYER is CODY; Captain Chowder John is the stationary advisor NPC on the main deck who guides you (characters swapped in the playtest-notes-2 session). Goal: get yourself (Cody) off the boat by completing minigames and a 4-step ritual (pipe → dinner → shower → nap) in the correct order. The pipe additionally requires VYOOSFRUMTHA CRYHOLE, won from the horse at the bar by answering its riddle. Fail 5 minigames = Cody becomes a hurricane and sinks Florida. Fail cutscene shows Cody on a coral throne as Aquaman.
...
- Trigger zones live in `rooms.js` as a `triggers` array on each room (`{ x, y, levelId }`). `OverworldScene` checks for trigger overlap after every move and calls `startMinigameForLevel`. Ritual levels are gated there: the step must be the next in ritual order (checked first, so probing a far-ahead ritual names the expected next step), the act's minigame must be in `completedMinigames` (`Objective.actMinigameDone`), and pipe-smoke ADDITIONALLY requires `'horse-riddle'` in `completedMinigames` (the VYOOSFRUMTHA CRYHOLE side-gate) — otherwise the trigger SOFT-BLOCKS with a `ritual-blocked-*` dialog (no failure, no hurricane) and `softBlockedAt` keeps the tile inert until the player steps off it. Marker language: yellow `!` = safe minigame, pulsing red `!` = ready ritual step, gray `?` = ritual that would refuse. `SequenceGuard.assertCanStartRitual` still runs last as a safety net but never fires in normal play.
...
- `src/systems/Objective.js` derives "what should the player do next" (act minigame first, then that act's ritual step; the pipe additionally routes through `horse-riddle` until the CRYHOLE is won) from the registry. Consumed by HUDScene (live `Objective:` line via `changedata-ritualProgress`/`completedMinigames`/`talkedToCody` subscriptions), and by OverworldScene (Cody's `cody-next-<levelId>` state-aware hints, ritual gating, trigger markers).
...
- Music is managed by `src/systems/MusicManager.js` — exports `playMusic(scene, key, volume)`, `stopMusic(scene)`, and `registerMusicKeepAlive(game)` (called once from `src/index.js`). Tracks current BGM via `game.registry` (`currentMusicKey`, `currentMusicInstance`, `currentMusicVolume`) so room transitions don't restart the overworld theme. Handles browser autoplay lock via a single pending-track `unlocked` listener. The keep-alive resumes the WebAudio context on any gesture/visibility gain (iOS suspends it on lock/calls/app-switch and doesn't reliably resume), plays a tiny looping silent HTML5 `<audio>` on the first gesture (the standard iOS SILENT-SWITCH workaround — Safari mutes all WebAudio in silent mode otherwise, the #1 "no music on my iPhone" cause), and runs a 4s watchdog that resumes a stuck-paused track or rebuilds a dead one — the BGM must always keep looping.
...
- (`src/ui/RhythmBar.js` was DELETED in the playtest-notes-2 session — scrolling-note timing windows were unreliable on phones. CokeDrinkGame is now a Simon-style memory game; LullabyGame is counting-sheep. Neither uses a shared UI class.)
...
Every character/tile creation uses `if (this.textures.exists(key)) { sprite } else { rectangle }`. Color flashes use `if (obj.setTint) { obj.setTint(color) } else { obj.setFillStyle(color) }` — Sprites have `setTint`, Rectangles have `setFillStyle`. Fallback colors: Player (Cody)=green (`COLORS.CODY`), Captain NPC=blue (`0x4060c0`), Horse=brown (`0x8b5a2b`), Mermaids=pink (`0xff69b4`), K-fish gold=yellow, K-fish red=red, Sheep=drawn white container, GenericNPCs=per-instance fallbackColor param. The horse/sheep/cryhole-juice sprites and bg-counting-sheep/bg-horse-riddle backgrounds are ON DISK (second Claude Design drop, pulled from the shared project via the browser); the drawn fallbacks remain as the safety net.
