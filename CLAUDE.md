...
Every character/tile creation uses `if (this.textures.exists(key)) { sprite } else { rectangle }`. Color flashes use `if (obj.setTint) { obj.setTint(color) } else { obj.setFillStyle(color) }` — Sprites have `setTint`, Rectangles have `setFillStyle`. Fallback colors: Player (Cody)=green (`COLORS.CODY`), Captain NPC=blue (`0x4060c0`), Horse=brown (`0x8b5a2b`), Mermaids=pink (`0xff69b4`), K-fish gold=yellow, K-fish red=red, Sheep=drawn white container, GenericNPCs=per-instance fallbackColor param. Expected-but-not-yet-on-disk art keys (loader warns, fallbacks used): horse, sheep, cryhole-juice sprites; bg-counting-sheep, bg-horse-riddle backgrounds.
